...
object TemplateRepo {
    private val templates = listOf(
        EmailTemplate(
            id = "auto-reply",
            name = "Auto Reply",
            description = "We'll be in touch — standard acknowledgement",
            htmlContent = AUTO_REPLY_HTML
        ),
        EmailTemplate(
            id = "followup",
            name = "Follow-up",
            description = "Good talking with you — next steps",
            htmlContent = FOLLOWUP_HTML
        ),
        EmailTemplate(
            id = "proposal",
            name = "Proposal",
            description = "Here's what we're proposing",
            htmlContent = PROPOSAL_HTML
        )
    )

    fun getTemplates(): List<EmailTemplate> = templates

    fun getById(id: String): EmailTemplate? = templates.find { it.id == id }

    fun buildCustomTemplate(name: String, message: String, cta: String = "Visit Synthrun →"): String {
        val displayName = if (name.isNotBlank()) "Hi $name," else "Hi there,"
        val safeMsg = message.replace("\n", "<br>")
        return CUSTOM_TEMPLATE(displayName, safeMsg, cta)
    }
}
...
private const val SHELL_OPEN = """<div style="background:#f2f1ee;padding:32px 12px;font-family:'Courier New',Courier,monospace;"><div style="max-width:560px;margin:0 auto;background:#fafaf8;border:1px solid #e0dfd9;">"""
private const val SHELL_CLOSE = "</div></div>"

private val AUTO_REPLY_HTML = """
$SHELL_OPEN
$HEADER
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  <tr>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <td style="padding:36px 28px 28px;">
      ...
      <p style="font-family:'Courier New',monospace;font-size:11px;line-height:1.85;color:#3a3a38;margin-bottom:0;">In the meantime, feel free to look around at what we've shipped at <a href="https://synthrun.site" style="color:#111110;">synthrun.site</a>.</p>
...
private fun CUSTOM_TEMPLATE(displayName: String, message: String, cta: String): String = """
  ...
  <div style="max-width:560px;margin:0 auto;background:#fafaf8;border:1px solid #e0dfd9;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr><td style="padding:36px 28px 32px;">
        ...
        <p style="font-family:'Courier New',monospace;font-size:11px;line-height:1.9;color:#3a3a38;">$message</p>
        <div style="margin-top:24px;">
          <a href="https://synthrun.site" style="display:inline-block;background:#111110;color:#fafaf8;text-decoration:none;font-family:'Courier New',monospace;font-size:9px;letter-spacing:0.12em;text-transform:uppercase;padding:10px 18px;">$cta</a>
        </div>
      </td></tr>
    </table>
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top:1px solid #e0dfd9;">
